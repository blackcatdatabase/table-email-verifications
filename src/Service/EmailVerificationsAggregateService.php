...
use ServiceHelpers;
    ...
    public function __construct(
        ...
        $this->repositories = [
  'email_verifications' => $this->emailVerificationRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\EmailVerifications\ModuleException("Record {$idStr} not found for locking.");
            }
