...
class EmailVerificationRepository implements EmailVerificationRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EmailVerifications\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'user_id', 'token_hash', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ]);
          return count($rows);
        }
    ...
    public function getBySelector(string $selector, bool $asDto = false): array|\BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto|null {
        return $this->getByUnique([ 'selector' => $selector ], $asDto);
    }
    ...
    public function getIdBySelector(string $selector) {
        $row = $this->getBySelector($selector, false);
