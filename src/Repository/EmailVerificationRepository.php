...
namespace BlackCat\Database\Packages\EmailVerifications\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EmailVerifications\Definitions;
use BlackCat\Database\Packages\EmailVerifications\Criteria;
use BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto as Dto;
use BlackCat\Database\Packages\EmailVerifications\Mapper\EmailVerificationDtoMapper as RowMapper;
...
final class EmailVerificationRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EmailVerifications\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'user_id', 'token_hash', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'selector' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EmailVerifications\Definitions::table());
            return $bulk->upsertMany($rows, [ 'selector' ], [ 'user_id', 'token_hash', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto|null */
    public function getBySelector(string $selector, bool $asDto = false): array|\BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto|null {
        $row = $this->getByUnique([ 'selector' => $selector ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EmailVerifications\Mapper\EmailVerificationDtoMapper::fromRow($row);
    }
    public function existsBySelector(string $selector): bool {
        $where = 't.' . Ident::q($this->db, 'selector') . ' = :uniq_selector';
        return $this->exists($where, [ 'uniq_selector' => $selector ]);
    }
    /** @return int|string|null */
    public function getIdBySelector(string $selector) {
        $row = $this->getBySelector($selector);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EmailVerifications\Dto\EmailVerificationDto|null {
