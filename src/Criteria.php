 ...
 * - whitelist filtrů: [ 'id', 'user_id', 'token_hash', 'selector', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ]
 * - whitelist pro LIKE hledání: [ 'token_hash', 'selector', 'key_version' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'user_id', 'token_hash', 'selector', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ], true);
        ...
        if ($this->search !== null) {
            $searchCols = [ 'token_hash', 'selector', 'key_version' ];
            $likeParts = [];
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'user_id', 'token_hash', 'selector', 'validator_hash', 'key_version', 'expires_at', 'created_at', 'used_at' ], true);
            $parts = [];
